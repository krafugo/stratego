...
- **Search** — it samples complete worlds from those beliefs (the enemy army as it imagines it, and its own army as the enemy would imagine it), searches each a few plies deep in a worker, and picks the move that does best on average. The enemy's replies are chosen under the enemy's own beliefs, never with knowledge of the computer's ranks: a known Major that walks up to a hidden Colonel is walking into a trap, and a hidden Captain beside it is in real danger — the search sees both. Attacks on unseen pieces are averaged over every rank the attacker's beliefs allow. The evaluation values ranks by what is left on the board (a Spy is worth much more while the enemy Marshal lives; the Miners are worth more the fewer remain, and the last one nearly a Marshal), chases pieces it can beat, keeps away from pieces that beat it, and closes in on the flag from anywhere on the board once the enemy army has thinned or the game has gone on long enough.
- **Habits of a careful player** — a few pieces do the work while the rest stay put, because every piece that moves for the first time tells the enemy it is not a bomb. It never attacks a revealed piece that would beat the attacker, and a piece left standing beside a known enemy that beats it is a cost the search always sees, so it gets away or strikes first. The Spy stays hidden, guards the General and stalks a revealed Marshal from two squares away, where the Marshal cannot strike first. Miners are the win condition: they stay home until the enemy army has thinned, never gamble on unknown movers, run from any officer that comes near, and the top pieces hunt intruders in the home rows. The Marshal and General never gamble on suspected bombs while a cheaper piece could probe.
