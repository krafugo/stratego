...
type Player = 'random' | 'weak' | 'bot' | 'red-opts' | 'blue-opts';
/** RED_OPTS / BLUE_OPTS: JSON BotOptions for the `red-opts` / `blue-opts` players, e.g. RED_OPTS='{"maxDepth":2}'. */
const OPTIONS: Record<Exclude<Player, 'random'>, BotOptions> = { weak: { timeMs: 20, samples: 2, maxDepth: 2 }, bot: { timeMs: 500, samples: 8, maxDepth: 4 }, 'red-opts': JSON.parse(process.env.RED_OPTS ?? '{}'), 'blue-opts': JSON.parse(process.env.BLUE_OPTS ?? '{}') };
...
for (let g = 0; g < games; g++) {
  ...
  while (host.view().phase === 'play' && moves < MAX_MOVES) {
    ...
    const move = kind === 'random' ? randomMove(mover) : chooseMove({ round: mover.round, sim: mover.sim(), me: mover.color, options: OPTIONS[kind] });
  ...
  console.log(`game ${g + 1}: ${result === 'draw' ? (v.phase === 'play' ? `unfinished after ${moves} moves` : 'draw') : `${result} wins by ${v.reason}`} in ${moves} moves · reversals red ${reversals.red} blue ${reversals.blue} · alive red ${alive.red} blue ${alive.blue} · captured from red ${cap.red.join(' ')} · from blue ${cap.blue.join(' ')}`);
}
