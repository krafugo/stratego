...
function aside(v: View) {
  ...
  const why = v.reason === 'flag' ? (v.outcome === 'win' ? 'You captured the enemy flag.' : 'Your flag was captured.') : v.reason === 'stuck' ? (v.outcome === 'win' ? `${friend} had no legal move left.` : 'You had no legal move left.') : v.reason === 'resigned' ? (v.outcome === 'win' ? `${friend} resigned.` : 'You resigned.') : v.reason === 'dead' ? 'Neither flag can be reached: both are sealed behind bombs and no Miners are left on either side.' : 'Both players resigned.';
